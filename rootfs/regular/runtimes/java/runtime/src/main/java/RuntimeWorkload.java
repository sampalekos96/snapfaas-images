...
public class RuntimeWorkload {
	...
	/* notify vmm of the boot completion */
	int cores = Runtime.getRuntime().availableProcessors();
        try {
            Process[] proc_list = new Process[cores];
            for (int i=1; i<cores; i++) {
                proc_list[i] = Runtime.getRuntime().exec(String.format("taskset -c %d outl 123 0x3f0", i));
            }
            proc_list[0] = Runtime.getRuntime().exec("taskset -c 0 outl 123 0x3f0");
            int exitVal = proc_list[0].waitFor();
            assert  exitVal == 0: "Err in signaling";
        }
        catch (Exception e) {
            System.out.println(e);
            System.exit(1);
